...
model04 = pickle.load(open('kid.pkl','rb'))
...
def kid():
    ...
    arr1 = arr.astype(float)
    pred = model04.predict(arr1)
    return render_template('afterkid.html',data=pred)
...
def upload():
    ...
    if pdf_file:
        ...
        datax = int(request.form['gett'])  # Convert datax to an integer
        ...
        if datax == 1:
            ...
            gender, age, smoking, yellow_fingers, anxiety_mean, peer_pressure, chronic_disease, fatigue, allergy, wheezing, alcohol_consumption, coughing, shortness_of_breath, swallowing_difficulty, chest_pain = parse_medical_report_lung(result_string)
            ...
            return render_template(
                ...
                age1=age,
                smoking1=smoking,
                yellow_fingers1=yellow_fingers,
                anxiety_mean1=anxiety_mean,
                peer_pressure1=peer_pressure,
                chronic_disease1=chronic_disease,
                fatigue1=fatigue,
                allergy1=allergy,
                wheezing1=wheezing,
                alcohol_consumption1=alcohol_consumption,
                coughing1=coughing,
                shortness_of_breath1=shortness_of_breath,
...
def parse_medical_report_lung(text):
    ...
    smoking_match = re.search(r'Smoking:\s*(\d+\.\d+)', text)
    smoking = float(smoking_match.group(1)) if smoking_match else 0.0

    yellow_fingers_match = re.search(r'Yellow Fingers:\s*(\d+\.\d+)', text)
    yellow_fingers = float(yellow_fingers_match.group(1)) if yellow_fingers_match else 0.0

    anxiety_match = re.search(r'Anxiety Mean:\s*(\d+\.\d+)', text)
    anxiety_mean = float(anxiety_match.group(1)) if anxiety_match else 0.0

    peer_pressure_match = re.search(r'Peer Pressure:\s*(\d+\.\d+)', text)
    peer_pressure = float(peer_pressure_match.group(1)) if peer_pressure_match else 0.0

    chronic_disease_match = re.search(r'Chronic Disease:\s*(\d+\.\d+)', text)
    chronic_disease = float(chronic_disease_match.group(1)) if chronic_disease_match else 0.0

    fatigue_match = re.search(r'Fatigue:\s*(\d+)', text)
    fatigue = int(fatigue_match.group(1)) if fatigue_match else 0

    allergy_match = re.search(r'Allergy:\s*(\d+)', text)
    allergy = int(allergy_match.group(1)) if allergy_match else 0

    wheezing_match = re.search(r'Wheezing:\s*(\d+)', text)
    wheezing = int(wheezing_match.group(1)) if wheezing_match else 0

    alcohol_consumption_match = re.search(r'Alcohol Consumption:\s*(\d+)', text)
    alcohol_consumption = int(alcohol_consumption_match.group(1)) if alcohol_consumption_match else 0

    coughing_match = re.search(r'Coughing:\s*(\d+)', text)
    coughing = int(coughing_match.group(1)) if coughing_match else 0

    breath_shortness_match = re.search(r'Shortness of Breath:\s*(\d+)', text)
    shortness_of_breath = int(breath_shortness_match.group(1)) if breath_shortness_match else 0

    swallowing_difficulty_match = re.search(r'Swallowing Difficulty:\s*(\d+)', text)
    swallowing_difficulty = int(swallowing_difficulty_match.group(1)) if swallowing_difficulty_match else 0

    chest_pain_match = re.search(r'Chest Pain:\s*(\d+)', text)
    chest_pain = int(chest_pain_match.group(1)) if chest_pain_match else 0
